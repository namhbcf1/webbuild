...
function evaluateSystemPerformance() {
    ...
    const selectedCpu = document.getElementById('cpu-dropdown')?.value || document.getElementById('cpu')?.value;
    const selectedGpu = document.getElementById('gpu-dropdown')?.value || document.getElementById('gpu')?.value;
    
    if (!selectedCpu || !selectedGpu) {
        console.warn('Chưa chọn CPU hoặc GPU. CPU:', selectedCpu, 'GPU:', selectedGpu);
        return;
    }
    ...
    console.log('Game ID được chọn:', selectedGameId);
    displayDetailedPerformance(selectedGameId);
    
    // Cập nhật tổng điểm
    const totalPerformance = document.getElementById('total-performance');
    if (totalPerformance) {
        const averageScore = Math.round((cpuScore + gpuScore) / 2);
        totalPerformance.textContent = averageScore + '/100';
        totalPerformance.style.color = getScoreColor(averageScore);
    }
    
    console.log('Đánh giá hiệu năng hoàn tất');
}
...
function updatePerformanceUI(cpuScore, gpuScore) {
    ...
    // Cập nhật biểu đồ (không cần truyền tham số vì updatePerformanceChart sẽ tự lấy giá trị từ các phần tử UI)
...
function updateLivestreamAndRenderPerformance(gamePerformance, graphicPerformance, officePerformance) {
    ...
    if (renderElement) {
        ...
        // Cập nhật score attribute cho span
...
function updatePerformanceChart(gamingValue, graphicsValue, officeValue, livestreamValue, renderValue, multitaskingValue) {
    console.log('Đang cập nhật biểu đồ hiệu năng với các giá trị:', {
        gaming: gamingValue,
    ...
    // Nếu không có tham số, lấy giá trị từ các phần tử UI
    if (typeof gamingValue === 'undefined') {
        const gameElement = document.getElementById('game-performance');
        gamingValue = gameElement ? parseInt(gameElement.getAttribute('data-value') || gameElement.style.width) : 0;
...
function calculateGamePerformance(cpuScore, gpuScore) {
    ...
    console.log('Tính toán hiệu năng gaming cho game type:', gameType);
    ...
    // Tính toán hiệu năng dựa trên trọng số
...
function calculateGraphicsPerformance(cpuScore, gpuScore) {
    ...
    // Đồ họa phụ thuộc chủ yếu vào GPU
...
function updateAllPerformanceMetrics() {
    ...
    // Tính toán các giá trị hiệu năng
    const gamingValue = calculateGamePerformance(cpuScore, gpuScore);
    const graphicsValue = calculateGraphicsPerformance(cpuScore, gpuScore);
    const officeValue = calculateOfficePerformance(cpuScore, gpuScore);
    
    console.log('Giá trị hiệu năng đã tính:', {
        gaming: gamingValue,
        graphics: graphicsValue,
        office: officeValue
    });
    ...
    try {
        // Cập nhật livestream và render
        const livestreamValue = updateLivestreamAndRenderPerformance(gamingValue, graphicsValue, officeValue);
        const renderValue = document.getElementById('render-performance').getAttribute('data-value') || 0;
        const multitaskingValue = Math.min(100, Math.round((cpuScore * 0.7 + 0.3 * (officeValue + renderValue) / 2)));
        
        // Cập nhật biểu đồ
        updatePerformanceChart(gamingValue, graphicsValue, officeValue, livestreamValue, renderValue, multitaskingValue);
        
        // Cập nhật phân tích
        updatePerformanceAnalysis(gamingValue, graphicsValue, officeValue, livestreamValue, renderValue, multitaskingValue);
        
        console.log('Các thành phần hiệu năng đã được cập nhật thành công');
    } catch (e) {
        console.error('Lỗi khi cập nhật metrics:', e);
    }
}
